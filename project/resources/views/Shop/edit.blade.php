 ...
 <body>
  <div class="mws-panel grid_8"> 
   ...
   <div class="mws-panel-body no-padding"> 
    <form class="mws-form" action="/admins/shop/update" method="post" enctype="multipart/form-data"> 
   <!--  @if(session('error'))
	...
	<div class="mws-form-message warning">
     ...
     <div class="mws-form-inline"> 
	  ...
	  <div class="mws-form-row">
		...
		<div class="mws-form-item">
			<script id="editor" type="text/plain"  name="descr" style="width:700px;height:500px;">{!!$info->name!!}</script>
		</div>
	</div>
  <div class="mws-form-row">
    ...
    <div class="mws-form-row"> 
       ...
       <div class="mws-form-item"> 
        <input class="small" type="text" name="num" value="{{$info->num}}"/> 
       </div> 
      </div>
      <div class="mws-form-row"> 
       <label class="mws-form-label">单价</label> 
       <div class="mws-form-item"> 
        <input class="small" type="text" name="price" value="{{$info->price}}"/> 
       </div> 
      </div> 
      <div class="mws-form-row">
          ...
          <div class="mws-form-item">
            <select class="small" name="cate_id">
              ...
              @foreach($cate as $row)
              <option value="{{$row->id}}" @if($info->cate_id == $row->id) selected @endif>{{$row->name}}</option>
              @endforeach
            </select>
          </div>
        </div> 
